...
describe('PeraWallet', () => {
  ...
  describe('resumeSession', () => {
    ...
    it('should resume session if session is found', async () => {
      ...
      expect(store.state.wallets[WalletId.PERA]).toEqual(walletAVMState)
      expect(wallet.isConnected).toBe(true)
    })
    ...
    it('should update the store if accounts do not match', async () => {
      ...
      store = new Store<AVMState>({
        ...defaultAVMState,
        wallets: {
          [WalletId.PERA]: prevWalletAVMState
        }
      })
      ...
      expect(store.state.wallets[WalletId.PERA]).toEqual(newWalletAVMState)
    })
